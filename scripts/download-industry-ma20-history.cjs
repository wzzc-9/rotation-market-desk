...
const configFile = process.env.INDUSTRY_MA20_CONFIG_FILE || 'config.json';
const config = JSON.parse(fs.readFileSync(path.join(strategyDirectory, configFile), 'utf8'));
const symbols = config.symbols;
const legacyHistoryPath = path.join(projectRoot, 'data', 'industry-ma20-history.json');
const onlyMissing = process.env.INDUSTRY_MA20_ONLY_MISSING === '1';
const ranges = [
