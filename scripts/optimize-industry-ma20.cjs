...
for (const symbol of symbols) {
  ...
  if (record.code !== symbol.code || !Array.isArray(record.rows) || record.rows.length < 21) throw new Error(`${symbol.code}.json 历史行情格式无效`);
  ...
  record.rows.forEach((row, index) => {
    ...
    const previousMa20 = record.rows.slice(index - 20, index).reduce((sum, item) => sum + Number(item[2]), 0) / 20;
    const previousClose = Number(record.rows[index - 1][2]);
    const averageVolume = record.rows.slice(index - 5, index).reduce((sum, item) => sum + Number(item[5]), 0) / 5;
    indicators.set(date, {
      close,
      ma20,
      momentum: close / Number(record.rows[index - 20][2]) - 1,
...
const warmupDates = dates.filter((date) => date < tenYearDates[0]);
...
for (let mask = 1; mask < 2 ** universe.length; mask += 1) {
  ...
  const initialPosition = simulate(codes, warmupDates, null, null).holding;
...
const result = {
  _comment: '页面“行业 ETF 20 日均线”的“全组合收益排名”表格数据，包括每个 ETF 组合的近10年、近5年、当年收益、回撤、综合得分和排名。',
  version: 'industry-ma20-combinations-volume15-daily-v1',
  strategy: 'industry-ma20',
  generatedAt: new Date().toISOString(),
  rule: { frequency: 'daily', momentumPeriod: 20, movingAveragePeriod: 20, minimumPoolSize: 3, initialPosition: 'previous-year-end-position', volumeRatioThreshold: 1.5 },
  periods: {
